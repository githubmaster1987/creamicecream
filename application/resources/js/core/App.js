...
.run(['$rootScope', '$state', 'users', 'utils', function($rootScope, $state, users, utils) {
    ...
    var statesThatNeedAuth = ['dashboard', 'admin', 'view'];
    ...
    $rootScope.$on('$stateChangeStart', function(e, toState, params) {
        ...
        if (stateName == 'admin') {
            if (! users.current || ! users.current.roles.admin) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }

        if(toState.name == "admin.folders"){
            if (! users.current || ! users.current.roles.folders) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }

        if(toState.name == "admin.roles"){
            if (! users.current || ! users.current.roles.roles) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }

        if(toState.name == "admin.files"){
            if (! users.current || ! users.current.roles.files) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }

        if(toState.name == "admin.settings"){
            if (! users.current || ! users.current.roles.settings) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }

        if(toState.name == "admin.users"){
            if (! users.current || ! users.current.roles.users) {
                e.preventDefault();
                $state.go('dashboard.folders');
            }
        }
