...
.factory('folders', ['$rootScope', '$http', '$mdDialog', '$stateParams', '$state', 'utils', 'dashboardState', function($rootScope, $http, $mdDialog, $stateParams, $state, utils, dashboardState) {
    var folders = {
        ...
        assignUsers: function(id) {
            console.log(this.folderNameModel);

            if(this.folderNameModel !== null && typeof this.folderNameModel === 'object')
            {
                /*this.folderNameModel = $.map(this.folderNameModel, function(value, index) {
                    return [value] = index;
                });*/
            }
            ...
            this.folderNameModel = this.folderNameModel.filter(function(n){ return n != undefined || n!= false }); 
            payload = { users: this.folderNameModel };
            if(this.folderNameModel){
                $http.post($rootScope.baseUrl+'folders/users/'+id, payload).success(function(data) {
                    utils.showToast('folderPermissionSuccess', true);
                    folders.closeModal();

                    for (var i = 0; i < folders.available.length; i++) {
                        if (data.id == folders.available[i].id) {
                            folders.available[i].users = data.users;
                            break;
                        }
                    }
        ...
        openPermissionModal: function(id, users, permission) {
            console.log(permission);
            this.folderNameModel = [];
            for (var key in permission) {
                if (!permission.hasOwnProperty(key)) continue;
                var per = permission[key];
                this.folderNameModel[per.user_id] = parseInt(per.user_id);
            }
            console.log(this.folderNameModel);
