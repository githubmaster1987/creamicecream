    ...
    var selectedItems = {
        ...
        permission: function () {
            ...
            if (this.first('type') === 'folder') {
                folders.usersList().success(function(users) {
                    folders.openPermissionModal($this.first('id'), users.permission, $this.first('users'));
                });
                
            } else {

                files.usersList().success(function(users) {
                    files.openPermissionModal($this.first('id'), users.permission);
                });
            }
        },
        ...
        download: function (items) {
            ...
            if ( ! items) {
                items = this.get().map(function(item) {
                    return { id: item.id, type: item.type, url: item.absoluteUrl, mime: item.mime, password: item.password }
                });
            }
            ...
            if (items.length === 1 && items[0].type === 'file') {
                console.log($rootScope.baseUrl+'download-file/'+items[0].id+'?password='+items[0].password);
                iframe.appendTo('body').attr('src', $rootScope.baseUrl+'download-file/'+items[0].id+'?password='+items[0].password);
            }
        ...
        /*permission: function() {
            $mdDialog.show({
                templateUrl: 'assets/views/modals/permission.html',
                clickOutsideToClose: true,
                controller: ['$scope', 'files', 'users', 'selectedItems', function($scope, files, users, selectedItems) {
                    $scope.selectedItems = selectedItems;
                    $scope.users = users.userslist();
console.log($scope.users);
                    // $scope.permission = function(userId) {
                    //     files.permissionToUser(selectedItems.get(), userId);
                    // }
                }]
            });
        },*/
